	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			// ---------- NL2SQL ----------
			{
				...
				type: 'options',
				noDataExpression: true,
				displayOptions: { show: { resource: ['nl2sql'] } },
				options: [
					{
						name: 'Ask',
						value: 'ask',
						description:
							'Turn a natural language question into SQL, and optionally run it ' +
							'(POST /v1/nl2sql/query)',
						action: 'Ask a natural language question',
					},
				],
				default: 'ask',
			},
			{
				...
				default: '',
				required: true,
				placeholder: 'What was the average requested budget across all destinations?',
				description: 'Your question in plain English -- SynapCores generates the SQL for it',
				displayOptions: { show: { resource: ['nl2sql'], operation: ['ask'] } },
			},
			{
				displayName: 'Database',
				name: 'nlDatabase',
				type: 'string',
				default: '',
				description: 'Optional. Scopes NL2SQL to a specific SynapCores database.',
				displayOptions: { show: { resource: ['nl2sql'], operation: ['ask'] } },
			},
			{
				displayName: 'Execute Generated SQL',
				name: 'nlExecute',
				type: 'boolean',
				default: true,
				description:
					'Whether to run the generated SQL immediately and return the actual query ' +
					'results, not just the SQL text. Turn off if you want to inspect/approve the ' +
					'generated SQL before it runs (e.g. as a review step before a write operation).',
				displayOptions: { show: { resource: ['nl2sql'], operation: ['ask'] } },
			},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'nl2sql' && operation === 'ask') {
					const query = this.getNodeParameter('nlQuestion', i) as string;
					const database = this.getNodeParameter('nlDatabase', i) as string;
					const executeSql = this.getNodeParameter('nlExecute', i) as boolean;

					const body: IDataObject = { query, execute: executeSql };
				...
				} else if (resource === 'ai' && operation === 'chat') {
